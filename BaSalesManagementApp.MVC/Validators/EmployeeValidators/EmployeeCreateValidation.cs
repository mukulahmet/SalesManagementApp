...
namespace BaSalesManagementApp.MVC.Validators.EmployeeValidators
{
    public class EmployeeCreateValidation : AbstractValidator<EmployeeCreateVM>
    {
        ...
        public EmployeeCreateValidation(IStringLocalizer<Resource> localizer)
        {
            ...
            RuleFor(x => x.Email)
                .NotEmpty().WithMessage(Messages.EMPLOYEE_E_MAIL_FIELD_IS_REQUIRED)
                .EmailAddress().WithMessage(Messages.EMPLOYEE_VALID_EMAIL_REQUIRED)
                .Matches(@"^[^@\s]+@[^@\s]+\.[^@\s]+$").WithMessage(Messages.EMPLOYEE_VALID_EMAIL_REQUIRED)
